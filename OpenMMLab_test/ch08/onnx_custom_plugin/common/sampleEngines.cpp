...
// sampleEngines.cpp - TensorRT Engine 构建核心实现
// 功能: 提供从各种模型格式(ONNX/Caffe/UFF)构建TensorRT Engine的完整流程
// 主要函数:
//   - getEngineBuildEnv(): 主入口函数,trtexec调用的核心构建函数
//   - modelToNetwork(): 将模型文件解析为TensorRT网络定义
//   - setupNetworkAndConfig(): 配置网络和构建选项
//   - networkToSerializedEngine(): 核心构建,将网络序列化为Engine
//   - saveEngine/loadEngine: Engine序列化/反序列化

#include <algorithm>      // 标准算法 (sort, find, transform等)
#include <chrono>        // 时间测量
...
// TensorRT 核心头文件
#include "NvInfer.h"       // TensorRT 主API
#include "NvOnnxParser.h" // ONNX 解析器头文件

// Samples 公共组件
#include "ErrorRecorder.h"  // 错误记录器
#include "common.h"        // 通用工具
#include "logger.h"        // 日志系统
#include "sampleDevice.h"  // CUDA设备管理
#include "sampleEngines.h" // Engine构建声明
#include "sampleOptions.h" // 选项定义
#include "sampleUtils.h"   // 工具函数

#if ENABLE_UNIFIED_BUILDER
#include "NvInferConsistency.h" // 一致性检查
#include "safeErrorRecorder.h"  // 安全错误记录器
#endif
...
// ==================== 辅助类: 文件流写入器 ====================
// 用于将序列化的Engine写入文件
// 实现 TensorRT 的 IStreamWriter 接口
class FileStreamWriter final : public nvinfer1::IStreamWriter
{
protected:
    std::ofstream mStream;      // 输出文件流
    int64_t mTotalWrittenSize;  // 已写入的总字节数

public:
    // 构造函数 - 打开文件准备写入
    // @param path: 要写入的文件路径
    FileStreamWriter(std::string const& path)
        : mStream(path, std::ios::binary)  // 以二进制模式打开
        , mTotalWrittenSize(0)
    {
    }
...
// ==================== 从校准缓存文件读取量化Scale ====================
// 用于 INT8 量化推理 - 读取预先计算好的 tensor scale 值
// @param calibrationFile: 校准缓存文件路径
// @return: map<tensor名称, scale值>
...
// ==================== LazilyDeserializedEngine: 延迟反序列化引擎 ====================
// 懒加载Engine - 第一次使用时才反序列化,节省内存
// 用于已序列化的Engine文件 (.trt)
...
nvinfer1::ICudaEngine* LazilyDeserializedEngine::get()
{
    ...
    if (mEngine == nullptr)
    {
        ...
        // 版本兼容模式: 允许加载包含Lean Runtime的Engine
...
// ==================== 设置Tensor Scale (INT8量化) ====================
// 从校准缓存文件读取scale值,并设置到网络的输入输出tensor
// @param network: 网络定义
// @param inputFormats: 输入格式 (FP32/FP16/INT8等)
// @param outputFormats: 输出格式
// @param calibrationFile: 校准缓存文件路径
    ...
    std::vector<IOFormat> const& outputFormats, std::string const& calibrationFile)
{
    ...
    for (int32_t i = 0, n = network.getNbInputs(); i < n; ++i)
    {
        ...
        if (!inputFormats.empty() && inputFormats[formatIdx].first == DataType::kINT8)
        {
            ...
            // INT8 范围: -127 ~ 127 (保留-128作为特殊值)
            input->setDynamicRange(-127 * calibScale, 127 * calibScale);
        }
    }

    // 设置输出tensor的dynamic range
...
//!
// ==================== modelToNetwork: 将模型文件解析为TensorRT网络 ====================
// 核心函数: 解析 ONNX/UFF/Caffe 模型文件,填充到 TensorRT 的 INetworkDefinition
// @param model: 模型选项 (文件路径、格式等)
// @param build: 构建选项
// @param network: 网络定义对象 (输出参数,解析结果填充到此)
// @param err: 错误输出流
// @param vcPluginLibrariesUsed: 输出参数,返回解析需要的VC插件库列表
// @return: Parser对象 (包含权重等信息)
    ...
    std::ostream& err, std::vector<std::string>* vcPluginLibrariesUsed)
{
    ...
    case ModelFormat::kONNX:
    {
        ...
        // 配置ONNX解析器选项
        // kNATIVE_INSTANCENORM 默认开启,需要清除才能使用插件实现
    ...
    case ModelFormat::kANY: break;  // 自动检测 (当前未实现)
    }
    ...
    sample::gLogInfo << "Finished parsing network model. Parse time: " << parseTime << " sec" << std::endl;
    return parser;
}
...
//!
// ==================== networkToSerializedEngine: 核心构建函数 ====================
// 将TensorRT Network序列化为Engine (核心构建过程)
// 这是真正执行模型优化和编译的地方
//
// 包含:
//   - 配置 BuilderConfig (精度、内存池、算法选择等)
//   - INT8 校准
//   - 调用 Builder 构建 Engine
//   - 性能计时
//
// @param build: 构建选项
// @param sys: 系统选项
// @param builder: IBuilder对象
// @param env: BuildEnvironment对象,存储构建结果
// @param err: 错误输出流
// @return: 成功返回true
    ...
    BuildOptions const& build, SystemOptions const& sys, IBuilder& builder, BuildEnvironment& env, std::ostream& err)
{
    ...
    // ==================== 步骤2: Timing Cache (加速重复构建) ====================
    std::unique_ptr<ITimingCache> timingCache{};
    if (!build.cpuOnly)
    {
        // 尝试从文件加载Timing Cache,不存在则创建新的
    ...
    // ==================== 步骤4: 执行构建 ====================
    // 分两种模式:
    // 1. 非安全模式 + 保存Engine: 直接写入文件,然后读取到内存
    // 2. 其他情况: 构建到内存
...
//!
// ==================== modelToBuildEnv: 从模型构建Engine ====================
// 核心构建函数: 从模型文件(ONNX等)构建TensorRT Engine
// 流程: 创建Builder -> 创建Network -> 解析模型 -> 序列化为Engine
//
// @param model: 模型选项 (文件路径、格式等)
// @param build: 构建选项
// @param sys: 系统选项
// @param env: BuildEnvironment对象,存储Builder/Network/Engine等
// @param err: 错误输出流
// @return: 成功返回true
    ...
    ModelOptions const& model, BuildOptions const& build, SystemOptions& sys, BuildEnvironment& env, std::ostream& err)
{
    ...
    // ==================== 步骤5: 核心构建: Network -> Serialized Engine ====================
    SMP_RETVAL_IF_FALSE(
...
// ==================== getEngineBuildEnv: Engine构建主入口函数 ====================
// trtexec.cpp 调用的核心函数,负责创建TensorRT Engine
// 根据选项决定是从模型构建还是从文件加载
//
// 执行流程:
//   1. 判断是从模型构建(load=false)还是从文件加载(load=true)
//   2. 构建/加载 Engine
//   3. 如果请求,保存Engine到文件
//
// @param model: 模型选项 (ONNX文件路径等)
// @param build: 构建选项 (是否save/load, fp16, int8等)
// @param sys: 系统选项 (设备ID, DLA等)
// @param env: BuildEnvironment对象,存储构建结果
// @param err: 错误输出流
// @return: 成功返回true
    ...
    ModelOptions const& model, BuildOptions const& build, SystemOptions& sys, BuildEnvironment& env, std::ostream& err)
{
    ...
    // ==================== 步骤1: 创建Engine ====================
    // 根据 build.load 判断是从模型构建还是从文件加载
    if (build.load)
    {
        // 从已序列化的Engine文件加载
        if (build.safe)
        {
            // 安全模式 + 加载: 从文件加载安全Engine
    ...
    if (build.save)
    {
        ...
        // 非安全模式: 释放内存blob,重新从文件加载
        // 这样可以确保Engine在文件系统上可被正确读取
        if (!build.safe)
        {
