...
int GatherTopk::enqueue(const nvinfer1::PluginTensorDesc *inputDesc,
  ...
  // Convert from int64_t to int (TensorRT 10 uses int64_t for dims)
  int dims_arr[8];
  int indices_dims_arr[8];
  int nbDims = inputDesc[0].dims.nbDims;
  int indice_nbDims = inputDesc[1].dims.nbDims;
  for (int i = 0; i < nbDims; ++i) {
    dims_arr[i] = static_cast<int>(inputDesc[0].dims.d[i]);
  }
  for (int i = 0; i < indice_nbDims; ++i) {
    indices_dims_arr[i] = static_cast<int>(inputDesc[1].dims.d[i]);
  }
  ...
  switch (data_type) {
    case nvinfer1::DataType::kFLOAT:
      gather_topk_impl<float>((float *)data, (int *)indices, dims_arr, nbDims, indices_dims_arr,
                              indice_nbDims, (float *)output, stream);
      break;

    case nvinfer1::DataType::kINT32:
      gather_topk_impl<int>((int *)data, (int *)indices, dims_arr, nbDims, indices_dims_arr, indice_nbDims,
                            (int *)output, stream);
      break;
    default:
      break;
