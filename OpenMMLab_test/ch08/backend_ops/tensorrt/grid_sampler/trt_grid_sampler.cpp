...
int TRTGridSampler::enqueue(const nvinfer1::PluginTensorDesc *inputDesc,
  ...
  int input_dims_arr[8];
  int grid_dims_arr[8];
  int output_dims_arr[8];
  for (int i = 0; i < input_dims.nbDims; ++i) {
    input_dims_arr[i] = static_cast<int>(input_dims.d[i]);
    grid_dims_arr[i] = static_cast<int>(grid_dims.d[i]);
    output_dims_arr[i] = static_cast<int>(output_dims.d[i]);
  }

  switch (data_type) {
    case nvinfer1::DataType::kFLOAT:
      grid_sample<float>((float *)outputs[0], (float *)inputs[0], (float *)inputs[1],
                         output_dims_arr, input_dims_arr, grid_dims_arr,
